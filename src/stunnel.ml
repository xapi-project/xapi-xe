  ...
  let waiter, pid =
        (fun () -> 
           (if wait then Unix.waitpid [] 
            else Unix.waitpid [Unix.WNOHANG]) x.pid),
        x.pid in
  let res = 
    try waiter ()
...
let attempt_one_connect ?unique_id ?(use_fork_exec_helper = true)
  ...
  (* Make sure we close config_in eventually *)
	...
	try
     ...
     | _ -> ())
  (fun () -> match config_in with Some fd -> Unix.close fd | _ -> assert false);
